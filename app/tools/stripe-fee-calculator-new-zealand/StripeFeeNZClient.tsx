...
function fmt(n: number) { return "NZ$" + n.toFixed(2); }
function parse(raw: string) { return parseFloat(raw.replace(/[^0-9.]/g, "")) || 0; }

export default function StripeFeeNZClient() {
    ...
    <div className="space-y-6">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1.5">
          {mode === "charge" ? "Amount you charge (NZD)" : "Amount you want to receive (NZD)"}
        </label>
        <div className="relative">
          <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 text-sm">NZ$</span>
      ...
      <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
        <div>
          <p className="text-sm font-medium text-gray-700">Include GST on fees (+15%)</p>
        ...
        <div className="bg-gray-50 rounded-xl border border-gray-100 p-5 space-y-2.5">
          {mode === "charge" ? (
            <>
              ...
              <div className="flex justify-between text-sm"><span className="text-gray-500">Stripe fee ({(card.pct * 100).toFixed(1)}% + NZ$0.30)</span><span className="font-mono text-red-500">− {fmt(baseFee)}</span></div>
              {includeGst && <div className="flex justify-between text-sm"><span className="text-gray-500">GST on fee (15%)</span><span className="font-mono text-red-400">− {fmt(gstOnFee)}</span></div>}
